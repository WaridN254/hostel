...
class Admin extends BaseController {
	...
	function ulogin()
	{
		...
		$password = html_escape($this->input->post('user_password'));
	...
	function encrypt_password_callback($post_array) {
		// Set default password for new branch admin users
		if (isset($post_array['user_type']) && $post_array['user_type'] === 'branch admin') {
			$post_array['user_pass'] = md5('Branch@123');
		} else {
			// For other user types, use the email as password (existing behavior)
