...
class Admin_model extends CI_Model {
	...
	function login($email, $password)
	{
		...
		try {
			...
			$adminquery = $this->loaginAdmin($email, $hashed_password);
		...
		// If admin login failed, try member login
		try {
			...
			if ($memberquery && $memberquery->num_rows() > 0) {
				...
				$newdata = array(
					...
					'user_name'   => $rows['ten_fname'] . ' ' . $rows['ten_lname'],
					'user_email'  => $rows['ten_email'],
					'user_type'   => 'member',
					'profphoto'   => !empty($rows['ten_pic']) ? $rows['ten_pic'] : 'default_avatar.png',
					'logged_in'   => TRUE,
				);
				$this->session->set_userdata($newdata);
				error_log('Member login successful for: ' . $email);
				return true;
			}
		} catch (Exception $e) {
	...
	function register_tenant($data)
	{
		error_log('Registering new tenant with data: ' . print_r($data, true));
		
		// Prepare tenant data
		$tenant_data = array(
			'ten_fname' => $data['fname'],
			'ten_lname' => $data['lname'],
			'ten_email' => $data['email'],
			'ten_phone' => $data['phone'],
			'ten_pass' => md5($data['password']), // MD5 hashing for compatibility
			'ten_created' => date('Y-m-d H:i:s'),
			'ten_isactive' => 1 // Auto-activate for now
		);
		
		// Insert into database
		$this->db->insert('tbl_tenants', $tenant_data);
		$tenant_id = $this->db->insert_id();
		
		error_log('New tenant registered with ID: ' . $tenant_id);
	...
	function loaginAdmin($email, $password)
	{
		...
		// Debug: Log the actual SQL query and result
		error_log('SQL Query: ' . $this->db->last_query());
		error_log('Found user: ' . ($query->num_rows() > 0 ? 'Yes' : 'No'));
		if($query->num_rows() > 0) {
			error_log('User data: ' . print_r($query->row_array(), true));
		}
	...
	{
		error_log('=== loginMember called ===');
		error_log('Email: ' . $email);
		
		// First, check if user exists and get their current status
		$this->db->where("ten_email", $email);
		$user = $this->db->get("tbl_tenants")->row_array();
		
		if ($user) {
			error_log('User found in database');
			error_log('User status (ten_isactive): ' . $user['ten_isactive']);
			
			// Check if account is active
			if ($user['ten_isactive'] != 1) {
				error_log('Login failed: Account is not active');
				return false;
			}
			
			error_log('Account is active');
			
			// Check password (comparing hashes)
			$hashed_password = md5($password);
			if ($user['ten_pass'] === $hashed_password) {
				error_log('Password matches');
				// Return the user data in the expected format
				$query = $this->db->where('ten_uid', $user['ten_uid'])->get('tbl_tenants');
				return $query;
			} else {
				error_log('Password mismatch');
				error_log('Expected: ' . $hashed_password);
				error_log('Got: ' . $user['ten_pass']);
				return false;
			}
		} else {
	...
	function changepassword($udata)
	{
		...
		else if($udata['user_type'] == 'admin' || $udata['user_type'] == 'branch admin')
		{
			$data = array(
